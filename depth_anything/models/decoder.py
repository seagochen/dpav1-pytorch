...
class DPTDecoder(nn.Module):
    ...
    架构:
    1. 调整层 (Resize): 调整特征图尺寸 (在编码器投影后)
    2. 投影层 (Projects): 使用 3x3 卷积将特征映射到融合维度
    3. 融合块 (Fusion): 渐进式特征融合
    4. 输出头 (Head): 生成最终深度图
    """

    def __init__(self, in_channels: List[int], features: int = 128,
                 use_bn: bool = False):
        """
        Args:
            in_channels: 各层输入通道数列表 (4个值)
            features: 中间特征通道数 (默认 128，匹配预训练权重)
        ...
        # 投影层 (scratch.layer_rn): 3x3 卷积将各层特征映射到 features 维度
        self.projects = nn.ModuleList([
            nn.Conv2d(in_channels[i], features, kernel_size=3, stride=1, padding=1, bias=False)
            for i in range(4)
        ])

        # 特征融合块 (scratch.refinenet): 渐进式融合
        ...
        # 输出头 (scratch.output_conv): 生成单通道深度图
        # 128 -> 64 -> 32 -> 1
    ...
    def forward(self, features: List[torch.Tensor]) -> torch.Tensor:
        ...
        # 1. 调整尺寸 (在投影之前)
        resized = [resize(feat) for resize, feat in zip(self.resize_layers, features)]

        # 2. 投影到统一维度 (scratch.layer_rn)
        projected = [proj(res) for proj, res in zip(self.projects, resized)]

        # 3. 渐进式融合 (从深层到浅层)
        # layer4 开始
        path = projected[3]  # layer4
        path = self.fusion_blocks[3](path)

        # 融合 layer3
        path = self.fusion_blocks[2](path, projected[2])

        # 融合 layer2
        path = self.fusion_blocks[1](path, projected[1])

        # 融合 layer1
        path = self.fusion_blocks[0](path, projected[0])
...
def build_decoder(in_channels: List[int], features: int = 128,
